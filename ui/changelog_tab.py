...
- **Expressive emotion prompt:** training saves the liveliest clean training clip beside the adapter (`<name>_expressive_reference.wav`), **Pick expressive clip** in the Voice LoRA / DoRA block does the same for an adapter trained earlier, and the new **Use the LoRA / DoRA expressive clip as the emotion prompt** checkbox (on by default) feeds it to the emotion pathway while the recommended reference keeps the identity. Measured on held-out sentences against the real recordings: fewer word errors and more pitch movement at unchanged identity; blind listening rounds were split (a hand-picked lively clip was preferred over the plain prompt, the automatically chosen one slightly not), so judge by ear and uncheck it, or change Emotion source, to hear the plain prompt.
- **Maximum pause (ms)** (Segmentation & Timing, default 0): a trained voice tends to reproduce the average pause density of its multi-sentence training clips on every sentence, and no decoding setting changes that. This control shortens every pause inside the finished audio that is longer than the value, keeping the words and the shorter pauses untouched; caption-timed output is never altered. In blind pairwise listening, a 300 ms cap was preferred over the uncapped output in 16 of 24 pairs with identity and word error unchanged, while 200 ms cut too much. **Max consecutive silence tokens** now states that the IndexTTS 2.5 codec has no repeated silence token, so it has no effect there.
