...
class IndexTTS2:
    def __init__(
            self, cfg_path="models/config.yaml", model_dir="models", use_bf16=False, device=None,
    ...
    def infer(self, spk_audio_prompt, text, output_path, lang="EN",
              ...
              max_consecutive_silence=0, semantic_layer=17, cfm_cache_length=8192,
              diffusion_steps=25, inference_cfg_rate=0.7,
              reset_beam_cache_per_segment=False, **generation_kwargs):
    ...
    def infer_texts(self, spk_audio_prompt, texts, on_text_complete=None, lang="EN", **kwargs):
        ...
        if section_batch_size == 1 or self.low_vram:
        ...
        cfm_cache_length = int(kwargs.pop("cfm_cache_length", 8192))
        ...
        for text_index, unit_text in enumerate(texts):
            processed_text = self.text_process.clean_pattern.sub(
                lambda match: self.text_process.char_rep_map[match.group()],
                unit_text,
            )
            if text_normalization:
                if lang_code.lower() in ['zh', 'zhen', 'en']:
                    processed_text = self.text_process.normalize(processed_text)
                elif lang_code.lower() in ['ja', 'es']:
                    processed_text = nemo_text_normalize(processed_text, lang_code.lower())
            if lang_code.lower() in ['ja', 'zh', 'zhen', 'en']:
                processed_text = processed_text.lower()
            if lang_code.lower() == 'es':
                processed_text = processed_text.upper()
            processed_text = apply_pronunciation_annotations(processed_text)
            if lang_code.lower() == 'ja':
                processed_text = self.ja_text_process.process_ja_text(processed_text)
            processed_text = re.sub(
                r'<\|([^|]+)\|>',
                lambda match: f'<|{match.group(1).upper()}|>',
                processed_text,
            )
            segments = self.split_text_by_tokens(
                processed_text,
        ...
        def emit_completed_text(text_index):
            ...
            if interval_silence > 0 and len(ordered_parts) > 1:
                silence = torch.zeros(
                    1,
                    int(sampling_rate * interval_silence / 1000.0),
                    dtype=torch.int16,
                )
                with_silence = []
                for part_index, part in enumerate(ordered_parts):
                    with_silence.append(part)
                    if part_index < len(ordered_parts) - 1:
                        with_silence.append(silence)
                ordered_parts = with_silence
            combined = torch.cat(ordered_parts, dim=1)
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast(
                    ...
                    self.s2mel.models['cfm'].estimator.setup_caches(
                        max_batch_size=cfm_batch_size,
                        max_seq_length=required_cache_length,
                    )
        ...
        for seg_idx, text_tokens in enumerate(segment_tokens):
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    if getattr(cfm_estimator, "_cached_seq_length", None) != int(cfm_cache_length):
                        cfm_estimator.setup_caches(
                            max_batch_size=1,
                            max_seq_length=int(cfm_cache_length),
                        )
