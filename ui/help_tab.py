...
1. **Prepare:** add media and sidecar captions in **LoRA Dataset Preparation**, scan the files, then prepare clean 24 kHz segments with the 4-16 second range and 14 second target, which Voice Generation later reproduces as its default line length for that voice. Sentence alignment automatically repacks complete sentences at source pauses and checks for at least 30 ms of quiet audio at both output edges after cleanup.
...
Speaking rate 1.0 is the model's natural pace; values below 1.0 speak more slowly and values above 1.0 speak faster. Training first estimates a completed LoRA / DoRA's pace from its epoch samples, then replaces that estimate with a calibration from matched held-out sentences once the automatic speech comparison finishes; **Calibrate speaking rate from this grid** can also measure a saved listening grid. Voice Generation auto-applies the saved value so the trained voice matches the words-per-second pace of its recordings, and the **Saved speaking rate for this LoRA / DoRA** field lets you override it per adapter with **Save speaking rate**. A manual value never discards the calibration: the adapter panel shows the calibrated (original) rate, the saved value and the slider's current value side by side, each with the words per second it produces, and the seconds in the line-length tables follow the slider as you move it.

## Line Length for a Trained Voice

A voice reproduces the clip lengths it was trained on. The **Voice LoRA / DoRA** panel therefore derives its rules from the adapter's training clips (`analysis/dataset_profile.json`, saved by training or measured on first selection): the **target** words per generated line are the middle of the training clips (40th to 60th percentile), the **acceptable range** covers 80 percent of them, the **hard limits** 90 percent, and **never exceed** is the longest clip. One generated line is one speech segment, so **Max tokens per segment** decides where lines end; with **Auto from LoRA / DoRA dataset** checked the app sets it so that sentences merge up to the training clips' typical length. A sentence shorter than the hard minimum should not stand alone, and a sentence longer than the segment budget is cut at a comma or a word boundary, so split very long sentences yourself. The **Use the LoRA / DoRA expressive clip as the emotion prompt** checkbox feeds the liveliest clean training clip (saved by training, or chosen with **Pick expressive clip**) to the emotion pathway while the recommended reference keeps the identity; it measured as the strongest single inference-side change on the objective side (fewer word errors, more pitch movement, identity unchanged) while blind listening rounds were split, and **Emotion weight** scales it. If the voice lingers at commas and sentence ends, **Maximum pause (ms)** in the Text & Timing block shortens every pause in the finished audio that exceeds it (300 ms measured best for brisk narration; 200 ms cut too much); the words themselves are not touched.

## Text Splitting and Pauses

**Text splitting** decides how long text becomes speech segments. **Smart sentences** (the default) packs whole sentences into each segment so every line lands near the selected voice's median training clip (the base model aims at 85 percent of the token limit); it never exceeds **Max tokens per segment**, cuts inside a sentence only when that sentence alone is longer than the limit, and never leaves a few words as the last segment. **Every sentence** renders one sentence per segment. **Token budget** is the former splitter, which cuts at any punctuation and can end a segment at a comma. The live preview shows the mode, the tokens, the words and the seconds of each section.

**Sentence pause (ms)** is the pause between two sentences the splitter separated, measured from the last spoken word to the next one: the model's own quiet tail and head count towards it, extra quiet is trimmed, and the rest is inserted as silence. 0 uses **Section silence** at every join. **Maximum pause (ms)** shortens every pause in the finished audio that is longer than the value; tagged `[pause:…]` silences and caption-timed output are never changed. With **Auto pauses from LoRA / DoRA dataset** checked, both values are taken from the speaker's training recordings when an adapter is selected: the median pause between sentences, and the length that only one in ten sentence pauses exceeds. The **Pauses of this speaker** card in the Voice LoRA / DoRA panel shows the measurements. Sentence pause is limited to Maximum pause when both are set.

**Repetition window (codes)** under Sampling limits the repetition penalty to the last N generated codes (25 codes are about one second). 0 keeps the model default, where the penalty applies to every code the segment has used so far and, above about 1.3, works as a ban.

## Pronunciation

The engine reads `<word|PHONES>` annotations: ARPAbet phones with stress digits and dots between syllables, for example `<Qwen|K W EH1 N>` or `<minute|M AY0 . N UW1 T>`. The **Pronunciation check & dictionary** accordion under the live section preview lists the words in your text that the selected voice never spoke in training and the base model has no dictionary reading for, proposes a reading (CMU dictionary, CamelCase and acronym splitting, letter-to-sound rules as the last resort), and keeps your dictionary in `pronunciations/dictionary.json`. **Add suggestions and save** adds the dictionary-backed readings; a low-confidence letter-rule guess stays in the check list for you to correct, because a wrong reading sounds worse than the plain spelling. Entries with scope `unseen` apply only to words outside the voice's training vocabulary, so readings the voice learned from its recordings are never replaced; `always` applies everywhere. A plain respelling such as `Comfy U I` replaces the word as text instead. The dictionary is applied to Voice Generation, Batch Generation and the live preview while **Apply the pronunciation dictionary when generating** is checked.
