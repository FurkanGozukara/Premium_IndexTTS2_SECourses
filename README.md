...
**V6.11 selection score and clip-length mix:** the speech comparison selects checkpoints by one deployment score (identity gain over Base, minus four times any word-error increase, plus a pause term), dataset preparation aims a quarter of the clips at single sentences and a quarter at medium clips by default (cut only between clear pauses), and training gains checkpoint averaging and decoder training on the GPT's own codes as options, both configurable and disabled by default. `tools/compare_checkpoint_benchmark.py` compares any checkpoint or decoder on a run's frozen benchmark.
...
Place the downloaded package in a short writable path such as `C:\Apps\IndexTTS`. Before the first install, provide Python 3.12.10 or newer, Git, FFmpeg, CUDA 13.0, cuDNN 9.17 or newer, and Visual Studio Community with the C++ workload. Then run `Windows_Install_or_Update.bat`.
...
**Smart sentences** and **Every sentence** treat line wrapping and blank lines between subtitle cues as spaces. A sentence can continue across multiple captions, and adding line breaks does not change its speech segments or token counts. Uploaded captions immediately use the selected splitting mode, voice target, pronunciation dictionary and speaking rate in the preview. **Use caption cue timing** still keeps the original timing units. Subtitles without sentence punctuation use the token limit as the fallback; line breaks alone do not establish sentence endings. Implementation and regression coverage: [subtitle sentence handling](docs/SMART_SENTENCES.md).
...
Token loss says how well a checkpoint predicts held-out audio codes, not how the voice sounds, so the **Epoch probe and two-signal early stopping** accordion (on by default) adds a second signal. After an epoch a separate process renders a few held-out sentences with the checkpoint using the deployment settings (three per held-out recording, 6 to 12, or a chosen count, two seeds each), renders Base the same way once, measures both against the real recordings (speaker similarity, word error, pauses) and scores the epoch with the deployment score. The best-scoring epoch is saved as `best/<name>_probe_best.safetensors`, listed as **probe-best**, and always joins the speech comparison. A validation-loss stall then stops training only when the probe's score has also stalled for **Probe patience** checks; while the probe still improves, training continues. A probe whose word error rises above the best check's by more than the recognizer's own error on the real recordings (or **Probe word-error tolerance**) for the patience count, while its score no longer improves, stops training as overfitting. Without probe results the loss decides as before. **Probe every N epochs** 0 spaces probes automatically so they cost at most a third of the training time on small, fast datasets; **Probe device** `auto` prefers another GPU with at least the minimum free VRAM, otherwise the training GPU when its free memory covers the minimum and the smallest generation tier's 5 GB budget (decided before any worker starts), so the training process never grows.
...
Before training, the app freezes held-out prompts, training-only voice references and deterministic generation seeds. After token-loss evaluation, it compares Base and up to three checkpoints from this run (plus the probe-best epoch, the averaged and EMA files when present) on six balanced prompts per held-out recording (12 to 24 by default, or a chosen count) plus a longer prompt per voice, using three seeds by default. With **Compare with the deployment settings** (on by default) every candidate is rendered the way Voice Generation deploys it: Base with the language defaults, the adapters with the GPU tier's beams and diffusion steps, Smart sentences at their token target, the dataset's pauses, their expressive clip and calibrated speaking rate, so the comparison measures what a user hears rather than fixed short-sample settings. Transcript errors, speaker similarity and failure checks screen regressions against Base; with the default **interval** guard a margin is disqualifying only when the paired 95% interval excludes zero or a majority of the held-out recordings show the regression, otherwise it is noted and costs score. Base can remain the recommendation. Among the candidates that pass those guards, one deployment score decides, the same score the voice decoder gate uses: the paired speaker-similarity gain over Base, minus four times any paired word-error increase (a point of word error costs 0.04 of similarity), plus a small term that rewards pausing more like the person than Base does; validation loss only breaks ties within 0.002. The report lists the score and its parts for every candidate, and each candidate's internal pause time relative to the person's recording of the same sentences, so a voice that rushes between sentences or lingers longer than the narrator is visible even when word error and pace match. **Use best checkpoint** follows the completed speech recommendation. **Average the last saved checkpoints** can add the parameter-space mean of the last N saved updates as one more candidate; it is off by default and selected only when the averaged candidate passes the same comparison.
...
Use `tools/gpu_tier_calibration.py --emulate` to check memory budgets across generation and the training pipeline. `--final-test` reserves another source recording for independent testing; `--final-test-dataset` supplies it for a train-only run. Explicit `--override training.*` settings apply after the stage defaults. The tool polls total GPU memory use, including other processes, and records each stage against its target budget. Probe workers fit their runtime tier to the memory available on the selected GPU.
...
**Held-out speech prompts** - `training.speech_eval_prompts`. 0 = automatic: six matched sentences per held-out recording, at least 12 and at most 24, so one recording cannot decide the comparison alone. Balanced across recordings, speakers and lengths; a longer prompt per voice is added when enough texts are available. *(default 0; minimum 0; maximum 100)*

**Compare with the deployment settings** - `training.speech_eval_deployment_settings`. Render every candidate the way Voice Generation deploys it by default (the GPU tier's beams and diffusion steps, Smart sentences with the adapter's token target and dataset pauses, its expressive clip and speaking rate; Base with the language defaults) instead of the fixed short-sample settings. *(default true)*

**Regression guard** - `training.speech_eval_guard_mode`. `interval`: a word-error increase or identity drop beyond the margin rejects a candidate only when its prompt-bootstrap 95% interval excludes zero or a majority of the held-out recordings show it; otherwise it is noted and costs deployment score. `mean`: the margin alone rejects. *(default "interval"; choices "interval", "mean")*

**Word-error weight in the deployment score** - `training.speech_eval_score_wer_weight`. One point of paired word-error increase costs this much divided by 100 of speaker similarity. *(default 4; minimum 0; maximum 100)*

**Probe each checkpoint with the deployment settings** - `training.probe_enabled`. After an epoch a separate process renders a few held-out sentences with the checkpoint using the deployment settings, measures them against the real recordings and scores the epoch against Base; the best-scoring epoch is kept as `best/<name>_probe_best.safetensors` and joins the speech comparison. *(default true)*

**Probe every N epochs** - `training.probe_every_epochs`. 0 = automatic: every epoch while a probe costs under a third of an epoch, otherwise spaced so probes stay under that share. *(default 0; minimum 0; maximum 10000)*

**Probe sentences** - `training.probe_prompts`. 0 = automatic: three per held-out recording, at least 6 and at most 12. *(default 0; minimum 0; maximum 50)*

**Probe seeds per sentence** - `training.probe_seeds`. Renders per sentence; two halve the swing one misread word gives a check. *(default 2; minimum 1; maximum 5)*

**Probe device** - `training.probe_device`. `auto`: another GPU with at least the minimum free VRAM when present, else the training GPU behind the epoch sample's free-VRAM gate; `same`: always the training GPU; or a device such as `cuda:1`. *(default "auto")*

**Let the probe confirm or defer early stopping** - `training.probe_stop_enabled`. A validation-loss stall stops training only when the probe's deployment score has also stalled; a probe whose word error degrades while its score stalls stops training as overfitting. Off keeps the probe informational. *(default true)*

**Probe patience (checks)** - `training.probe_patience`. Consecutive probe checks without a deployment-score gain above the minimum before the probe counts as stalled. *(default 2; minimum 1; maximum 100)*

**Probe minimum improvement** - `training.probe_min_delta`. Deployment-score gain over the best check that counts as progress. *(default 0.002; minimum 0; maximum 1)*

**Probe word-error tolerance** - `training.probe_wer_tolerance`. 0 = automatic: the recognizer's own error on the real probe recordings, at least 0.01. *(default 0; minimum 0; maximum 1)*

**Probe timeout (s)** - `training.probe_timeout_s`. A probe that runs longer is killed and skipped; training continues. *(default 900; minimum 60)*
...
**Judge the decoder with the best adapter even when Base leads** - `training.decoder_adapter_always_gate`. When the speech comparison preferred Base, the voice decoder adapter is still tested through the full pipeline with the best-scoring adapter checkpoint, and adapter + decoder is judged against Base and every plain adapter by the same guards and deployment score; it becomes the recommendation only when it wins. *(default true)*
